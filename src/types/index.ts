...
export interface RewriteConfig {
  ...
  styles: Array<`synonym` | `sentence` | `expand` | `tone`>
  retains: Array<`quote` | `term` | `location` | `number`>
  extras: Array<`seo` | `readability` | `originality`>
}
...
// 改写风格配置
export type RewriteStyle = `synonym` | `sentence` | `expand` | `tone`

// 保留原文配置
export type RetainType = `quote` | `term` | `location` | `number`

// 其他功能配置
export type ExtraFeature = `seo` | `readability` | `originality`
